...
class CustomInfoWindowState extends State<CustomInfoWindow> {
  ...
  void _updateInfoWindow() async {
    ...
    double devicePixelRatio = Theme.of(context).platform == TargetPlatform.android ? MediaQuery.of(context).devicePixelRatio : 1.0;
  ...
  void _addInfoWindow({required Widget child, required LatLng latLng, double? offset, double? height, double? width}) {
    _child = child;
    _latLng = latLng;
    _offset = offset;
    _height = height;
